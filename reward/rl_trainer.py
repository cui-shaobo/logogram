...
class RLTrainer(object):
    ...
    def build_dataloader(self, args):
        self.train_dataloader = get_loader(self.train_jsonl_file, self.model_mode, self.train_batch_size, args.t5_model,
                                           args.max_length,
                                           args.max_decode_step)
        self.val_dataloader = get_loader(self.val_jsonl_file, self.model_mode, self.val_batch_size, args.t5_model,
                                         args.max_length,
                                         args.max_decode_step)

    def loss(self, out):
        # need to customize the loss function for reward and the regular cross entropy function
        pass
    ...
    def train(self):
        ...
        for epoch in range(1, self.train_num_epochs + 1):
            ...
            for batch_idx, batch in enumerate(self.train_dataloader, start=1):
                ...
                labels = batch['labels']
                out = self.model(encoder_inputs=input_ids, decoder_labels=labels)
